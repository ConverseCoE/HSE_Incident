...
const ApprovalPanel = ({ onViewIncident }) => {
  ...
  const totalPendingItems = pendingInitialReview.length + pendingActionVerifications.length + pendingExtensionRequests.length + pendingClosureSignoff.length;
  ...
  return (
    <div className="approval-panel-layout animate-fade" style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '0px' }}>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'left' }}>
          <h2 style={{ fontSize: '1.15rem', fontWeight: 700, margin: '0 0 4px 0', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            Enterprise Approvals & Governance Center
          </h2>
          <p className="text-muted" style={{ fontSize: '0.82rem', margin: 0 }}>
            Sequential approval workflow for triage, CAPA verification, due-date extensions, and final closure sign-offs.
          </p>
        </div>
      ...
      {/* 4-Column Sequential Approvals Grid */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', textAlign: 'left', alignItems: 'start' }}>
        
        {/* COLUMN 1: INITIAL INCIDENT REVIEW & TRIAGE */}
        <div style={{ background: '#ffffff', borderRadius: '12px', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', background: '#f8fafc', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Initial classification</span>
            </div>
              ...
              pendingActionVerifications.map(act => (
                ...
                >
                  ...
                  {canVerifyCapa ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.7rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>
                      <Lock size={12} /> HSE Manager / Admin Signature Required
                    </div>
                  )}
                </div>
              ))
        ...
        {/* COLUMN 3: DUE-DATE EXTENSION APPROVALS */}
        <div style={{ background: '#ffffff', borderRadius: '12px', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', background: '#f8fafc', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h3 style={{ fontSize: '0.88rem', fontWeight: 700, margin: 0, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              pendingExtensionRequests.map(req => (
                ...
                >
                  ...
                  {canApproveExtension ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.7rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>
                      <Lock size={12} /> HSE Manager Approval Required
                    </div>
                  )}
                </div>
              ))
            ...
            {pendingClosureSignoff.length === 0 ? (
              ...
              pendingClosureSignoff.map(inc => (
                ...
                >
                  ...
                  {!canFinalSignoff && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.7rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>
                      <Lock size={12} /> HSE Manager / Executive Signoff Required
                    </div>
                  )}
                </div>
              ))
